...
import {
  ...
  Clock, MapPin, X, Save, Trash2, Edit3
} from 'lucide-react';
...
const INITIAL_EVENTS = [
  { id:1,  date:'2025-01-06', title:'Semester 1 Begins',                      type:'term',    time:'07:00',       venue:'School Grounds'        },
  ...
  { id:6,  date:'2025-03-17', title:'HOD Meeting — Semester 1 Review',         type:'meeting', time:'10:00',       venue:'Conference Room'        },
  { id:7,  date:'2025-03-24', title:'Semester 1 Exams Begin',                  type:'exam',    time:'08:00',       venue:'Exam Halls A, B, C'    },
  { id:8,  date:'2025-04-04', title:'Semester 1 Exams End',                    type:'exam',    time:'16:00',       venue:'Exam Halls A, B, C'    },
  { id:9,  date:'2025-04-11', title:'Semester 1 Ends',                         type:'term',    time:'12:00',       venue:'School Grounds'         },
  { id:10, date:'2025-04-14', title:'Semester 2 Begins',                       type:'term',    time:'07:00',       venue:'School Grounds'         },
  ...
  { id:14, date:'2025-05-12', title:'WASSCE Begins',                            type:'exam',    time:'08:30',       venue:'AFSHTS Centre GH0042'  },
  { id:15, date:'2025-06-13', title:'Semester 2 Exams Begin',                  type:'exam',    time:'08:00',       venue:'Exam Halls A, B, C'    },
  { id:16, date:'2025-07-04', title:'Graduation Ceremony — Form 3',             type:'event',   time:'10:00',       venue:'Main Hall'              },
  { id:17, date:'2025-07-25', title:'Semester 2 Ends',                          type:'term',    time:'12:00',       venue:'School Grounds'         },
];
...
const EventModal = ({ event, onSave, onClose }) => {
  ...
  const IField = ({ label, field, type='text', placeholder, required }) => (
    <div>
      ...
      {errors[field] && <p className="text-xs mt-1" style={{ color:'var(--accent-red)' }}>{errors[field]}</p>}
    </div>
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md flex flex-col overflow-hidden">
        ...
        <div className="p-6 space-y-4 overflow-y-auto">
          <IField label="Event Title" field="title" placeholder="e.g. End of Semester Exams" required/>
          <IField label="Date"        field="date"  type="date" required/>
          <IField label="Time"        field="time"  placeholder="e.g. 08:00 or 09:00–12:00 or All Day"/>
          <IField label="Venue / Location" field="venue" placeholder="e.g. Main Hall, School Field"/>

          {/* Type selector */}
          <div>
            <label className="text-xs font-bold uppercase tracking-wider block mb-1.5"
              style={{ color:'var(--dark-gray)' }}>Event Type</label>
          ...
          {form.title && form.date && (
            <div className="p-3 rounded-xl border"
              ...
              <p className="text-xs font-bold mb-0.5" style={{ color:ts.color }}>Preview</p>
              ...
              <div className="flex flex-wrap gap-3 mt-1">
                ...
                {form.time  && <p className="text-xs text-gray-500 flex items-center gap-1"><Clock size={10}/> {form.time}</p>}
                {form.venue && <p className="text-xs text-gray-500 flex items-center gap-1"><MapPin size={10}/> {form.venue}</p>}
              </div>
            </div>
          )}
        </div>
...
const AdminCalendar = () => {
  ...
  const getEventsForDate = (day) => {
    ...
    return events.filter(e => e.date===ds && (filterType==='all' || e.type===filterType));
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border shadow-sm overflow-hidden"
          ...
          <div className="flex items-center justify-between px-5 py-3.5 border-b"
            style={{ borderColor:'var(--medium-gray)', backgroundColor:'var(--light-gray)' }}>
            <button type="button" onClick={prevMonth} className="p-1.5 rounded-lg hover:bg-gray-200">
              <ChevronLeft size={16} style={{ color:'var(--dark-gray)' }}/>
            </button>
            <h2 className="font-black text-base" style={{ color:'var(--dark-gray)' }}>
              {MONTHS[month]} {year}
            </h2>
            <button type="button" onClick={nextMonth} className="p-1.5 rounded-lg hover:bg-gray-200">
          ...
          <div className="grid grid-cols-7">
            {Array.from({length:firstDay}).map((_,i)=>(
              <div key={`e-${i}`} className="border-b border-r p-2 min-h-[70px]"
                style={{ borderColor:'var(--medium-gray)' }}/>
            ))}
            {Array.from({length:daysInMonth}).map((_,i)=>{
              ...
              return (
                <div key={day}
                  ...
                  style={{ borderColor:'var(--medium-gray)', backgroundColor:isSel?'#eef2ff':undefined }}
                  ...
                  <div className="flex items-center justify-between mb-0.5">
                    ...
                    <button type="button"
                      onClick={e=>{ e.stopPropagation(); openForDate(day); }}
        ...
        <div className="space-y-5">
          ...
          {selectedDate && (
            <div className="bg-white rounded-xl border shadow-sm overflow-hidden"
              ...
              <div className="flex items-center justify-between px-4 py-3 border-b"
                ...
                <button type="button" onClick={()=>openForDate(selectedDate)}
                      ...
                      <div key={e.id} className="p-3">
                        <div className="flex items-start gap-2">
                          ...
                          <div className="flex items-center gap-1 flex-shrink-0">
                            <button type="button" onClick={()=>{ setEditEvent(e); setShowModal(true); }}
          ...
          <div className="bg-white rounded-xl border shadow-sm p-4"
            ...
            <p className="text-xs font-bold uppercase tracking-widest mb-3"
              style={{ color:'var(--dark-gray)', opacity:0.5 }}>Legend</p>
