...
import {
  ...
  getGradeColor, getGradeLabel, getPerformanceBand,
...
const BarTooltip = ({ active, payload, label }) => {
  if (active && payload?.length) {
    return (
      <div className="bg-white rounded-xl shadow-lg border px-3 py-2"
        style={{ borderColor: 'var(--medium-gray)', fontSize: '12px' }}>
        <p className="font-bold mb-0.5" style={{ color: 'var(--dark-gray)' }}>{label}</p>
        <p style={{ color: '#6b7280' }}>CA: <strong>{payload.find(p=>p.dataKey==='ca')?.value ?? '—'}</strong></p>
        <p style={{ color: 'var(--royal-blue)' }}>Exam: <strong>{payload.find(p=>p.dataKey==='exam')?.value ?? '—'}</strong></p>
...
const StudentResults = () => {
  ...
  const terms = [
    { key: 'current', label: `${CURRENT_RESULTS.term} · ${CURRENT_RESULTS.academicYear}` },
    ...PREVIOUS_RESULTS.map((r, i) => ({ key: `prev_${i}`, label: `${r.term} · ${r.academicYear}` })),
  ];

  const active      = selectedTerm === 'current' ? CURRENT_RESULTS : PREVIOUS_RESULTS[parseInt(selectedTerm.split('_')[1])];
  ...
  const radarData = active.subjects.map(s => ({
    subject: s.name.replace('Integrated ', 'Int. ').replace(' Language', '').replace(' Studies', ''),
    score: s.total,
    fullMark: 100,
  }));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-xl font-black" style={{ color: 'var(--dark-gray)' }}>My Results</h1>
          <p className="text-xs text-gray-400 mt-0.5">{user?.formClass} · {user?.program}</p>
        </div>
      ...
      <div className="rounded-2xl p-5 text-white relative overflow-hidden"
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <p className="text-blue-200 text-xs mb-0.5">{active.term} · {active.academicYear}</p>
            <p className="font-black text-2xl">{band.label}</p>
            <p className="text-blue-200 text-sm">
              Overall: {percentage}% · Position: {active.position} of {active.totalStudents}
            </p>
          </div>
          <div className="flex gap-3">
            ...
            <div className="text-center px-4 py-2 rounded-xl" style={{ backgroundColor: 'rgba(255,255,255,0.12)' }}>
              <p className="font-black text-xl">
                {active.subjects.filter(s => ['A1','B2','B3','C4','C5','C6'].includes(s.grade)).length}
              </p>
      ...
      {activeTab === 'table' && (
        <div className="bg-white rounded-xl border shadow-sm overflow-hidden"
          ...
          <div className="flex items-center justify-between px-5 py-3.5 border-b"
            ...
            <h3 className="font-semibold text-sm" style={{ color: 'var(--dark-gray)' }}>Subject Results</h3>
          ...
          {showKey && (
            <div className="px-5 py-4 border-b grid grid-cols-3 sm:grid-cols-9 gap-2"
              style={{ borderColor: 'var(--medium-gray)', backgroundColor: '#f8f9fa' }}>
              {GRADE_KEY.map(g => (
                <div key={g.grade} className="text-center p-1.5 rounded-lg"
                  style={{ backgroundColor: 'white', border: '1px solid var(--medium-gray)' }}>
                  <span className={`text-xs font-black ${getGradeColor(g.grade)} px-1.5 py-0.5 rounded`}>{g.grade}</span>
              ...
              <tbody>
                <tr>
                  <td colSpan={7} className="px-4 py-1.5 text-xs font-black uppercase"
                    style={{ backgroundColor: '#eef2ff', color: 'var(--royal-blue)' }}>Core Subjects</td>
                </tr>
                {active.subjects.filter(s => s.type === 'Core').map((sub, i) => (
                  <tr key={i} className="border-b hover:bg-gray-50" style={{ borderColor: 'var(--medium-gray)' }}>
                    ...
                    <td className="px-4 py-3"><span className="text-xs px-1.5 py-0.5 rounded font-semibold" style={{ backgroundColor: '#eef2ff', color: 'var(--royal-blue)' }}>Core</span></td>
                ...
                <tr>
                  <td colSpan={7} className="px-4 py-1.5 text-xs font-black uppercase"
                    style={{ backgroundColor: '#f0fdf4', color: 'var(--success-dark)' }}>Elective Subjects</td>
                </tr>
                {active.subjects.filter(s => s.type === 'Elective').map((sub, i) => (
                  <tr key={i} className="border-b hover:bg-gray-50" style={{ borderColor: 'var(--medium-gray)' }}>
                    <td className="px-4 py-3 font-medium" style={{ color: 'var(--dark-gray)' }}>{sub.name}</td>
                    <td className="px-4 py-3"><span className="text-xs px-1.5 py-0.5 rounded font-semibold" style={{ backgroundColor: '#f0fdf4', color: 'var(--success-dark)' }}>Elective</span></td>
                    <td className="px-4 py-3 text-center">{sub.ca}</td>
                    <td className="px-4 py-3 text-center">{sub.exam}</td>
                    <td className="px-4 py-3 text-center font-black" style={{ color: 'var(--royal-blue)' }}>{sub.total}</td>
                    <td className="px-4 py-3"><span className={`px-2 py-0.5 rounded text-xs font-black ${getGradeColor(sub.grade)}`}>{sub.grade}</span></td>
      ...
      {activeTab === 'charts' && (
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl border shadow-sm p-5"
            ...
            <p className="text-xs text-gray-400 mb-4">Breakdown of your scores per subject</p>
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={barData} margin={{ top: 5, right: 10, left: -20, bottom: 50 }}>
                ...
                <ReferenceLine y={50} stroke="var(--accent-red)" strokeDasharray="4 4" />
                <Bar dataKey="ca"    name="CA Score"    fill="#94a3b8" radius={[4,4,0,0]} />
                <Bar dataKey="exam"  name="Exam Score"  fill="var(--royal-blue)" radius={[4,4,0,0]} />
                <Bar dataKey="total" name="Total Score" fill="var(--success-dark)" radius={[4,4,0,0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-xl border shadow-sm p-5"
              ...
              <p className="text-xs text-gray-400 mb-2">How balanced are your scores?</p>
              <ResponsiveContainer width="100%" height={240}>
                <RadarChart data={radarData}>
                  <PolarGrid stroke="#e5e7eb" />
                  <PolarAngleAxis dataKey="subject" tick={{ fontSize: 10, fill: '#6b7280' }} />
                  <PolarRadiusAxis angle={30} domain={[0, 100]} tick={{ fontSize: 9 }} />
                  <Radar name="Score" dataKey="score" stroke="var(--royal-blue)"
                    fill="var(--royal-blue)" fillOpacity={0.25} />
            ...
            <div className="bg-white rounded-xl border shadow-sm p-5"
              ...
              <p className="text-xs text-gray-400 mb-2">Your performance across all terms</p>
              <ResponsiveContainer width="100%" height={240}>
                <LineChart data={trendData} margin={{ top: 10, right: 20, left: -20, bottom: 30 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                  <XAxis dataKey="term" tick={{ fontSize: 10, fill: '#6b7280' }} angle={-25} textAnchor="end" />
                  <YAxis tick={{ fontSize: 10, fill: '#6b7280' }} domain={[40, 100]} />
                  <Tooltip formatter={(v, n) => [n === 'avg' ? `${v}%` : `#${v}`, n === 'avg' ? 'Avg Score' : 'Position']}
                    contentStyle={{ fontSize: 12, borderRadius: 8 }} />
                  <Legend wrapperStyle={{ fontSize: 12, paddingTop: 8 }} />
                  <ReferenceLine y={50} stroke="var(--accent-red)" strokeDasharray="4 4" />
                  <Line type="monotone" dataKey="avg" name="Avg Score (%)" stroke="var(--royal-blue)"
                    strokeWidth={2.5} dot={{ r: 5, fill: 'var(--royal-blue)' }} activeDot={{ r: 7 }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
