...
import {
  BookOpen, ClipboardList, FileText, CalendarCheck,
  Bell, ChevronRight, TrendingUp, Award, RefreshCw,
...
import {
  TERM_INFO, CURRENT_RESULTS, PREVIOUS_RESULTS,
  ATTENDANCE_SUMMARY, STUDENT_NOTIFICATIONS,
  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-white rounded-xl shadow-lg border px-3 py-2"
        ...
        <p className="font-bold" style={{ color: 'var(--dark-gray)' }}>{label}</p>
        <p style={{ color: 'var(--royal-blue)' }}>Score: <strong>{payload[0].value}</strong></p>
      </div>
    );
  }
  return null;
...
const StudentHome = () => {
  ...
  const totalScore  = CURRENT_RESULTS.subjects.reduce((s, sub) => s + sub.total, 0);
  const maxScore    = CURRENT_RESULTS.subjects.length * 100;
  const percentage  = ((totalScore / maxScore) * 100).toFixed(1);
  const totalPoints = CURRENT_RESULTS.subjects.reduce((s, sub) => s + sub.points, 0);
  ...
  // Term trend data
  const allTerms = [
    ...PREVIOUS_RESULTS.slice().reverse(),
    CURRENT_RESULTS,
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl p-5 sm:p-6 text-white relative overflow-hidden"
        ...
        <div className="relative z-10 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div>
            ...
            <h2 className="text-xl sm:text-2xl font-black">{user?.firstName} {user?.lastName}</h2>
            <p className="text-blue-300 text-xs mt-0.5">{user?.studentId} · {user?.formClass}</p>
            <p className="text-blue-300 text-xs">{user?.program} · Track {user?.track}</p>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { icon: BookOpen,     label: 'Subjects',       value: CURRENT_RESULTS.subjects.length, sub: `${CURRENT_RESULTS.subjects.filter(s=>s.type==='Core').length} core · ${CURRENT_RESULTS.subjects.filter(s=>s.type==='Elective').length} elective`, color: 'var(--royal-blue)', path: '/student/results' },
          { icon: TrendingUp,   label: 'Overall Score',  value: `${percentage}%`,                sub: band.label,                                         color: band.color,              path: '/student/results'    },
          { icon: Award,        label: 'Class Position', value: `${CURRENT_RESULTS.position}/${CURRENT_RESULTS.totalStudents}`, sub: CURRENT_RESULTS.term, color: 'var(--warning)',        path: '/student/results'    },
          { icon: CalendarCheck,label: 'Attendance',     value: `${attPct}%`,                    sub: `${ATTENDANCE_SUMMARY.present}/${ATTENDANCE_SUMMARY.totalDays} days`, color: attColor, path: '/student/attendance' },
        ...
        <div className="lg:col-span-2 bg-white rounded-xl border shadow-sm p-5"
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">Pass mark: 50 · Credit: 50+</p>
            </div>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={subjectChartData} margin={{ top: 5, right: 10, left: -20, bottom: 40 }}>
              ...
              <ReferenceLine y={50} stroke="var(--accent-red)" strokeDasharray="4 4"
                label={{ value: 'Pass (50)', position: 'insideTopRight', fontSize: 10, fill: 'var(--accent-red)' }}
              />
              <ReferenceLine y={80} stroke="var(--success-dark)" strokeDasharray="4 4"
                label={{ value: 'A1 (80)', position: 'insideTopRight', fontSize: 10, fill: 'var(--success-dark)' }}
              />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <BarChart data={trendData} margin={{ top: 5, right: 10, left: -20, bottom: 5 }}>
              ...
              <Bar dataKey="avg" fill="var(--royal-blue)" radius={[6, 6, 0, 0]}>
                {trendData.map((entry, i) => (
                  <Cell
                    key={i}
                    fill={i === trendData.length - 1 ? 'var(--accent-red)' : 'var(--royal-blue)'}
        ...
        <div className="bg-white rounded-xl border shadow-sm overflow-hidden"
          ...
          <div className="flex items-center justify-between px-5 py-3.5 border-b"
            ...
            <button onClick={() => navigate('/student/attendance')}
              className="text-xs flex items-center gap-1" style={{ color: attColor }}>
          ...
          <div className="p-4">
            <div className="grid grid-cols-2 gap-3 mb-4">
              {[
                { label: 'Present', value: ATTENDANCE_SUMMARY.present, color: 'var(--success-dark)' },
                { label: 'Absent',  value: ATTENDANCE_SUMMARY.absent,  color: 'var(--accent-red)'   },
                { label: 'Late',    value: ATTENDANCE_SUMMARY.late,     color: 'var(--warning)'      },
            ...
            <div>
              <div className="flex justify-between text-xs mb-1">
                <span style={{ color: 'var(--dark-gray)', fontWeight: 600 }}>Attendance Rate</span>
        ...
        <div className="bg-white rounded-xl border shadow-sm p-5"
          ...
          <h3 className="font-semibold text-sm flex items-center gap-2 mb-4"
            style={{ color: 'var(--dark-gray)' }}>
            <RefreshCw size={15} style={{ color: 'var(--royal-blue)' }} /> Double Track
          </h3>
          <div className="space-y-3">
            <div className="rounded-xl p-4 border" style={{ backgroundColor: '#fefce8', borderColor: '#fde68a' }}>
              ...
              <p className="text-xs" style={{ color: '#92400e' }}>
                {new Date(TERM_INFO.startDate).toLocaleDateString('en-GB', { day:'numeric', month:'short', year:'numeric' })} –{' '}
                {new Date(TERM_INFO.endDate).toLocaleDateString('en-GB',   { day:'numeric', month:'short', year:'numeric' })}
              </p>
              <p className="text-xs font-semibold mt-1" style={{ color: '#78350f' }}>
                Week {TERM_INFO.weeksGone}/{TERM_INFO.weeksTotal}
              </p>
            </div>
